...
const AddProject = () => {
  ...
  const [head, setHead] = useState("Add Project");
  const [location, setLocation] = useState("");
  const [projectManager, setProjectManager] = useState("");
  const [totalEmpInProject, setTotalEmpInProject] = useState("");
  ...
  const saveProject = (p) => {
    ...
    if (projectId) {
      ...
      projectService
        .update(project)
        .then((response) => {
          console.log(projectName, location, projectManager, totalEmpInProject);
    ...
    if (projectId) {
      ...
      projectService
        ...
        .then((project) => {
          ...
          setProjectManager(project.data.projectManager);
          setTotalEmpInProject(project.data.totalEmpInProject);
          setHead("Update Project");
        })
        .catch((error) => {
          console.log("Something went wrong", error);
        });
  ...
  const projectNameInputValid = !projectNameValid && projectNameTouched;
  const locationInputValid = !locationValid && locationTouched;

  const projectNameStyle = projectNameInputValid
  ...
  return (
          ...
          >
            <div className="w3-twothird">
              ...
              >
                ...
                <form>
                  <div className="form-group">
                    <input
                      ...
                      onBlur={projectBlurName}
                      placeholder="Enter Project name"
                    />
                    {projectNameInputValid && (
                      <p
                  ...
                  <div className="form-group">
                    <select
                      ...
                      onChange={(d) => setProjectManager(d.target.value)}
                    >
                ...
                <hr />
                <Link to="/projectList">Back to List</Link>
              </div>
            </div>
          </div>
        </div>
    ...
    //         id="projectName"
    //         value={projectName}
    //         onChange={(p) => setProjectName(p.target.value)}
    //         placeholder="Enter Project name"
    //       />
    //     </div>
    //     <div className="form-group">
    ...
    //     </div>
    //     <div>
    //       <button onClick={(p) => saveProject(p)} className="btn btn-primary">
    //         Save
    //       </button>
    //     </div>
    //   </form>
